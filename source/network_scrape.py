...
def main(root_user='FactoryBerlin'):
    ...
    # graph.persist_graph('RobertVoigtDE', 'RobertVoigtDE')
...
def pull_remote_graph(screen_name, scope_limit, twitter_function, edge_check_function, edge_function):
    user_object = session.query(Node).filter_by(screen_name=screen_name).first()
    next_cursor = -1
...
def pull_remote_graph_friend(screen_name, scope_limit=1):
    ...
    if (len(user_object.reference_nodes()) < 5):
        pull_remote_graph(screen_name, scope_limit, twitter.get_friends_list, edge_check_point, edge_point)


def pull_remote_graph_follow(screen_name, scope_limit=1):
    user_object = session.query(Node).filter_by(screen_name=screen_name).first()
    if (len(user_object.pointer_nodes()) < 5):
        pull_remote_graph(screen_name, scope_limit, twitter.get_followers_list, edge_check_reference, edge_reference)
