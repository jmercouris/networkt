"""
+ Extended graphs of each Transnational User Graph
...
for node in tag.users:
    ...
    for follower in node.followers:
        ...
        for nested_friend in follower.friends:
            graph.add_node(nested_friend.screen_name, sanitize_dict(vars(nested_friend)))
            graph.add_edge(nested_friend.screen_name, follower.screen_name)
        
        for nested_follower in follower.followers:
            graph.add_node(nested_follower.screen_name, sanitize_dict(vars(nested_follower)))
            graph.add_edge(follower.screen_name, nested_follower.screen_name)
    ...
    for friend in node.friends:
        ...
        for nested_friend in friend.friends:
            graph.add_node(nested_friend.screen_name, sanitize_dict(vars(nested_friend)))
            graph.add_edge(nested_friend.screen_name, friend.screen_name)
        
        for nested_follower in friend.followers:
            graph.add_node(nested_follower.screen_name, sanitize_dict(vars(nested_follower)))
            graph.add_edge(friend.screen_name, nested_follower.screen_name)
